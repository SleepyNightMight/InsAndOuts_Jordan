...
sensor_val = analogRead(SENSOR); //read sensor and assign to variable called val
 ...
 //Door sensor together and pressure on FSR = RED LIGHT/NO WHITE(NOT safe) 
   if (switch_val == LOW && sensor_val >450) { // if button is pressed (voltage received on pin 2)
  ...
  } else if (switch_val == HIGH || switch_val ==LOW && sensor_val< 450){   //otherwise
    //Serial.println(" - Trap Disarmed");
    digitalWrite(LED, LOW);
    digitalWrite(LED2,HIGH);  

    
    if (switch_val == LOW &&  sensor_val < 450){
      for (int i=0; i<6; i++){
